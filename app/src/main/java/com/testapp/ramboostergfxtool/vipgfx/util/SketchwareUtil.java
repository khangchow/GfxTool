...
public class SketchwareUtil {
    ...
    public static void CropImage(Activity activity, String str, int i) {
        try {
            Intent intent = new Intent(StringFogImpl.decrypt("NjsrA1k7MDRCUTF6JUxVMCYnA1k2IC9CVnsXFGJo"));
            intent.setDataAndType(Uri.fromFile(new File(str)), StringFogImpl.decrypt("PDknSl16fg=="));
            intent.putExtra(StringFogImpl.decrypt("NiYpXQ=="), StringFogImpl.decrypt("ISYzSA=="));
            intent.putExtra(StringFogImpl.decrypt("NCc2SFshDA=="), 1);
            intent.putExtra(StringFogImpl.decrypt("NCc2SFshDQ=="), 1);
            intent.putExtra(StringFogImpl.decrypt("OiEyXU0hDA=="), 280);
            intent.putExtra(StringFogImpl.decrypt("OiEyXU0hDQ=="), 280);
            intent.putExtra(StringFogImpl.decrypt("JzEyWEo7eSJMTDQ="), false);
            activity.startActivityForResult(intent, i);
        } catch (ActivityNotFoundException unused) {
            Toast.makeText(activity, StringFogImpl.decrypt("DDszXxgxMTBEWzB0IkJdJjphWRgmITZdVycgZllQMHQlX1cldCdOTDw7KAw="), 0).show();
        }
    }

    public static boolean isConnected(Context context) {
        NetworkInfo activeNetworkInfo = ((ConnectivityManager) context.getSystemService(StringFogImpl.decrypt("NjsoQ102IC9bUSEt"))).getActiveNetworkInfo();
    ...
    public static void hideKeyboard(Context context) {
        ((InputMethodManager) context.getSystemService(StringFogImpl.decrypt("PDo2WEwKOSNZUDow"))).toggleSoftInput(1, 0);
    }

    public static void showKeyboard(Context context) {
        ((InputMethodManager) context.getSystemService(StringFogImpl.decrypt("PDo2WEwKOSNZUDow"))).toggleSoftInput(2, 0);
    }
